...
> This skill runs in pi, which has no automatic PostToolUse hook. You MUST run the
> validation pipeline manually after writing the file. `SKILL_DIR` is the directory
> this SKILL.md lives in (default install: `~/.pi/agent/skills/aws-architecture-diagram`).
> The validators use `defusedxml`, so run them through `uv run --with` (no global install needed).

1. Write the `.drawio` file to `./docs/`
2. Run post-processing fixers (badge overlaps, nesting, icon colors, external-actor placement):

   ```bash
   uv run --with "defusedxml>=0.7.1" python3 ~/.pi/agent/skills/aws-architecture-diagram/scripts/lib/post_process_drawio.py ./docs/<filename>.drawio
   ```

3. Validate the XML (structure, AWS shapes, edges, geometry). Look for `VALIDATION PASSED`:

   ```bash
   uv run --with "defusedxml>=0.7.1" python3 ~/.pi/agent/skills/aws-architecture-diagram/scripts/lib/validate_drawio.py ./docs/<filename>.drawio
   ```

4. If validation fails, fix the reported errors, rewrite the file, and re-run steps 2-3.
5. After validation passes, generate a browser preview URL:

   ```bash
   python3 ~/.pi/agent/skills/aws-architecture-diagram/scripts/lib/drawio_url.py ./docs/<filename>.drawio --open
   ```
...
5. After writing, run the validation pipeline manually (see Step 4 above):
   a. `post_process_drawio.py` to apply fixers
   b. `validate_drawio.py` and confirm it prints `VALIDATION PASSED`
6. If validation fails, fix the errors and rewrite the file, then re-run the pipeline
